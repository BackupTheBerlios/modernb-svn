...
int ActivateSubContainers(BOOL active)
{
  ...
  //if (active) SetForegroundWindow(hwndContactList);
  return 0;
}
int SetParentForContainers(HWND parent)
{
  int i;
  if (parent&&parent!=hwndContactList)
    IsOnDesktop=1;
  else
    IsOnDesktop=0;
  for(i=0;i<nFramescount;i++){
...
int OnShowHide(HWND hwnd, int mode)
{
  ...
  if (mode!=SW_HIDE) SetForegroundWindow(hwndContactList);
  return 0;
}
static int RemoveItemFromList(int pos,wndFrame **lpFrames,int *FrameItemCount)
{
...
static int CLUIFramesRemoveFrame(WPARAM wParam,LPARAM lParam)
{
  ...
  */	
  ...
  ulockfrm();
  InvalidateRectZ(hwndContactList,NULL,TRUE);
  CLUIFramesOnClistResize((WPARAM)hwndContactList,0);
  InvalidateRectZ(hwndContactList,NULL,TRUE);
...
int OnFrameTitleBarBackgroundChange()
{
  ...
  //		if (hwndStatus) InvalidateRectZ(hwndStatus,NULL,TRUE);
  ...
  if (pos>=0&&pos<nFramescount)
  {
      ...
      if (!AlignCOLLIconToLeft)
      {
        ...
        if(Frames[pos].TitleBar.hicon!=NULL)	{
          ...
          TextOutS(hdcMem,rc.left+GetSystemMetrics(SM_CXSMICON)+4,rc.top+2,Frames[pos].TitleBar.tbname,MyStrLen(Frames[pos].TitleBar.tbname));
        }
        else
          TextOutS(hdcMem,rc.left+2,rc.top+2,Frames[pos].TitleBar.tbname,MyStrLen(Frames[pos].TitleBar.tbname));

      }else
      //if (!Frames[pos].floating){
        TextOutS(hdcMem,rc.left+GetSystemMetrics(SM_CXSMICON)+2,rc.top+2,Frames[pos].TitleBar.tbname,MyStrLen(Frames[pos].TitleBar.tbname));
      //}
...
{
  HWND hwnd;
  hwnd=CreateWindowEx(MySetLayeredWindowAttributesNew?WS_EX_LAYERED:0,CLUIFrameSubContainerClassName,"aaaa",WS_POPUP|WS_BORDER/*|WS_THICKFRAME*/,x,y,width,height,parent,0,g_hInst,0);
