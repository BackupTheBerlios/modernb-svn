...
int LoadSkinModule()
{
  ...
  if (gdiPlusFail)
  {
	  ...
	  if (hImageDecoderModule==NULL) 
	{
		...
		GetModuleFileName( g_hInst, tDllPath, sizeof( tDllPath ));
		{
...
BOOL SkFillRectangle(HDC hDest, HDC hSource, RECT * rFill, RECT * rGlyph, RECT * rClip, BYTE mode, BYTE drawMode, int depth)
{
	...
	else if (mode==FM_TILE_VERT && (rGlyph->bottom-rGlyph->top>0)&& (rGlyph->right-rGlyph->left>0))
	{
		...
		if (!oldbmp) 
			return 0;
		//MessageBox(NULL,"Tile bitmap not selected","ERROR", MB_OK);
		/// draw here
		{
			int  y=0, sy=0, maxy=0;
...
BOOL DeleteBitmap32(HBITMAP hBmpOsb)
{
	...
	if (!res) 
	{
		//     MessageBox(NULL,"IMAGE STILL SELECTED","IMAGE DELETING ERROR",MB_OK); 
		DebugBreak();
	}
    ...
    //UpdateAlphaCannel
    {
      ...
      for (y=0; y<heigh;y++)
      {
        ...
        for(x=0; x<width; x++)
        {
          ...
          gx=pix[1];
          rx=pix[2];
          ax=(BYTE)((bx+rx+gx)/3);                     // alpha on new should be mid of three components
          ax=weight[ax];                               // gamma correction of alpha level 
          if (ax)                                      
          {
            /*
			if (b<bufpix[0]) bx=weight[bx];            // Next we had some visual effects... at dark on light we should correct collors too
            if (g<bufpix[1]) gx=weight[gx];            // otherwise black font will be bolder, errors are due to we paint mask as white on black
			...
			*/
            ...
            bufpix[2]=(BYTE)(((DWORD)(rx*r+bufpix[2]*(BYTE)(~rx)))/255);			
            bufpix[3]=(BYTE)(ax+(BYTE)(~ax)*bufpix[3]/255);
          }
        }
      }
