...
//	
//	FirstIconOffset=DBGetContactSettingDword(NULL,"StatusBar","FirstIconOffset",0);
...
//				if (!IsBadStringPtr(PD->RealName, 2)) mir_free(PD->RealName);
//				if (PD) mir_free(PD);
//			}
//			}
//	}
...
//	
////--??	SendMessage(hwndStatus,SB_SETBKCOLOR,0,DBGetContactSettingDword(0,"StatusBar","BkColour",CLCDEFAULT_BKCOLOUR)); 
...
//				if (DBGetContactSettingDword(0,"Protocols",(char *)&buf,1)==0){continue;};
//
//				partWidths[part]=((rc.right-rc.left-FirstIconOffset*2)/toshow)*(part+1)+FirstIconOffset;
//                    //(part+1)*rc.right/toshow-(borders[2]>>1)+(FirstIconOffset*(part==0));
...
//		InvalidateRectZ(hwndStatus,NULL,FALSE);
//	
//	return 0;
//}
//
