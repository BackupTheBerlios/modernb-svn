...
static int CreateGroup(WPARAM wParam,LPARAM lParam)
{
	...
	else lstrcpynA(newBaseName,Translate("New Group"),sizeof(newBaseName));
	itoa(newId,str,10);
	i=1;
	lstrcpynA(newName+1,newBaseName,sizeof(newName)-1);
...
static int GetGroupName2(WPARAM wParam,LPARAM lParam)
{
	...
	if((DWORD*)lParam!=NULL) *(DWORD*)lParam=dbv.pszVal[0];
	DBFreeVariant(&dbv);
	return (int)name;
}
...
static int DeleteGroup(WPARAM wParam,LPARAM lParam)
{
	...
	if(pszLastBackslash) pszLastBackslash[0]='\0';
	else szNewParent[0]='\0';
	hContact=(HANDLE)CallService(MS_DB_CONTACT_FINDFIRST,0,0);
	do {
		if(DBGetContactSetting(hContact,"CList","Group",&dbv)) continue;
		if(MyStrCmp(dbv.pszVal,name)) {DBFreeVariant(&dbv); continue;}
		DBFreeVariant(&dbv);
		if(szNewParent[0]) DBWriteContactSettingString(hContact,"CList","Group",szNewParent);
	...
	{	char szNewName[256];
		...
		for(i=0;;i++) {
			...
			if(!strncmp(dbv.pszVal+1,name,len) && dbv.pszVal[len+1]=='\\' && strchr(dbv.pszVal+len+2,'\\')==NULL) {
				if(szNewParent[0]) _snprintf(szNewName,sizeof(szNewName),"%s\\%s",szNewParent,dbv.pszVal+len+2);
				else lstrcpynA(szNewName,dbv.pszVal+len+2,sizeof(szNewName));
...
static int RenameGroupWithMove(int groupId,const char *szName,int move)
{
	...
	if(GroupNameExists(szName,groupId)) {
		MessageBoxA(NULL,Translate("You already have a group with that name. Please enter a unique name for the group."),Translate("Rename Group"),MB_OK);
		return 1;
	}

	//do the change
	itoa(groupId,idstr,10);
	if(DBGetContactSetting(NULL,"CListGroups",idstr,&dbv)) return 1;
	str[0]=dbv.pszVal[0];
	lstrcpynA(oldName,dbv.pszVal+1,sizeof(oldName));
	DBFreeVariant(&dbv);
	lstrcpynA(str+1,szName,sizeof(str)-1);
	...
	do {
		...
		if(MyStrLen(cacheEntry->szGroup)==0) continue;
		if(MyStrCmp(cacheEntry->szGroup,oldName)) continue;
	...
	if(move) {
		...
		lstrcpynA(str,szName,sizeof(str));
		...
		*pszLastBackslash='\0';
		for(i=0;;i++) {
			itoa(i,idstr,10);
			if(DBGetContactSetting(NULL,"CListGroups",idstr,&dbv)) break;
			if(!lstrcmpA(dbv.pszVal+1,str)) {
				if(i<groupId) break;	//is OK
...
static int SetGroupExpandedState(WPARAM wParam,LPARAM lParam)
{
	...
	itoa(wParam-1,idstr,10);
	if(DBGetContactSetting(NULL,"CListGroups",idstr,&dbv)) return 1;
	if(lParam) dbv.pszVal[0]|=GROUPF_EXPANDED;
	else dbv.pszVal[0]=dbv.pszVal[0]&~GROUPF_EXPANDED;
...
static int MoveGroupBefore(WPARAM wParam,LPARAM lParam)
{
	...
	if(wParam==0 || (LPARAM)wParam==lParam) return 0;
	itoa(wParam-1,str,10);
	if(DBGetContactSetting(NULL,"CListGroups",str,&dbv)) return 0;
	szMoveName=dbv.pszVal;
	//shuffle list of groups up to fill gap
	if(lParam==0) {shuffleFrom=wParam-1; shuffleTo=-1; shuffleDir=-1;}
	else {
		if((LPARAM)wParam<lParam) {shuffleFrom=wParam-1; shuffleTo=lParam-2; shuffleDir=-1;}
		else {shuffleFrom=wParam-1; shuffleTo=lParam-1; shuffleDir=1;}
	}
	if(shuffleDir==-1) {
		for(i=shuffleFrom;i!=shuffleTo;i++) {
			itoa(i+1,str,10);
			if(DBGetContactSetting(NULL,"CListGroups",str,&dbv)) {shuffleTo=i; break;}
			itoa(i,str,10);
			DBWriteContactSettingString(NULL,"CListGroups",str,dbv.pszVal);
			DBFreeVariant(&dbv);
		}
		...
		for(i=shuffleFrom;i!=shuffleTo;i--) {
			itoa(i-1,str,10);
			if(DBGetContactSetting(NULL,"CListGroups",str,&dbv)) {mir_free(szMoveName);DBFreeVariant(&dbv); return 1;}  //never happens
			itoa(i,str,10);
			DBWriteContactSettingString(NULL,"CListGroups",str,dbv.pszVal);
			DBFreeVariant(&dbv);
		}
...
static int BuildGroupMenu(WPARAM wParam,LPARAM lParam)
{
	...
	if(DBGetContactSetting(NULL,"CListGroups","0",&dbv)) return (int)(HMENU)NULL;
	DBFreeVariant(&dbv);
	...
	for(groupId=0;;groupId++) {
		...
		do {
			...
			if(pBackslash==NULL) {
				lstrcpynA(szThisField,pNextField,sizeof(szThisField));
				pNextField=NULL;
			}
			else {
				lstrcpynA(szThisField,pNextField,min(sizeof(szThisField),pBackslash-pNextField+1));
			...
			for(menuId=0;menuId<menuItemCount;menuId++) {
				...
				compareResult=lstrcmpA(szThisField,szThisMenuItem);
				if(compareResult==0) {
					if(pNextField==NULL) {
						mii.fMask=MIIM_DATA;
				...
				if((int)mii.dwItemData-1>groupId) break;
			}
			if(compareResult) {
