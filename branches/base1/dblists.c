...
int i;
	...
	TRACE("End Dumping List\r\n");
}

SortedList* List_Create( int p_initial_value, int p_increment )
{
	SortedList* result = ( SortedList* )calloc( sizeof( SortedList ), 1 );
	if ( result == NULL )
		return(NULL);

	ZeroMemory( result, sizeof( SortedList ) );

	result->increment = p_increment;

	if (p_initial_value > 0)
	{
		result->items = ( void** )calloc( sizeof( void* ), p_initial_value );
	}
	result->limit = p_initial_value;
...
void List_DestroyWithContents( SortedList* p_list )
{
	if ( p_list == NULL )
		return;
