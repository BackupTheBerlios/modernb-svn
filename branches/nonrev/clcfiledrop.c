...
static STDMETHODIMP_(ULONG) CDropTarget_Release(struct CDropTarget *lpThis)
{
	if(lpThis->refCount==1) {
		if(lpThis->pDropTargetHelper) lpThis->pDropTargetHelper->lpVtbl->Release(lpThis->pDropTargetHelper);
	}
	return --lpThis->refCount;
}
...
static STDMETHODIMP_(HRESULT) CDropTarget_DragOver(struct CDropTarget *lpThis,DWORD fKeyState,POINTL pt,DWORD *pdwEffect)
{
	...
	if(lpThis->pDropTargetHelper && hwndCurrentDrag) lpThis->pDropTargetHelper->lpVtbl->DragOver(lpThis->pDropTargetHelper,(POINT*)&pt,*pdwEffect);
	...
	*pdwEffect=0;
	...
	if(dat->selection!=hit) {
		dat->selection=hit;
		InvalidateRectZ(hwndCurrentDrag,NULL,FALSE);
...
static STDMETHODIMP_(HRESULT) CDropTarget_DragEnter(struct CDropTarget *lpThis,IDataObject *pData,DWORD fKeyState,POINTL pt,DWORD *pdwEffect)
{
	...
	if(lpThis->pDropTargetHelper && hwndCurrentDrag) lpThis->pDropTargetHelper->lpVtbl->DragEnter(lpThis->pDropTargetHelper,hwndCurrentDrag,pData,(POINT*)&pt,*pdwEffect);
...
static STDMETHODIMP_(HRESULT) CDropTarget_DragLeave(struct CDropTarget *lpThis)
{
	if(hwndCurrentDrag) {
		struct ClcData *dat;
		if(lpThis->pDropTargetHelper) lpThis->pDropTargetHelper->lpVtbl->DragLeave(lpThis->pDropTargetHelper);
		dat=(struct ClcData*)GetWindowLong(hwndCurrentDrag,0);
		dat->showSelAlways=0;
...
static STDMETHODIMP_(HRESULT) CDropTarget_Drop(struct CDropTarget *lpThis,IDataObject *pData,DWORD fKeyState,POINTL pt,DWORD *pdwEffect)
{
	...
	if(lpThis->pDropTargetHelper && hwndCurrentDrag) lpThis->pDropTargetHelper->lpVtbl->Drop(lpThis->pDropTargetHelper,pData,(POINT*)&pt,*pdwEffect);
	...
	*pdwEffect=DROPEFFECT_NONE;
	...
	if(stg.pUnkForRelease) stg.pUnkForRelease->lpVtbl->Release(stg.pUnkForRelease);
	else GlobalFree(stg.hGlobal);
